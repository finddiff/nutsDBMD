...
func TestSkipListBytes(t *testing.T) {
	a := assert.New(t)
	//list := New(Float64)
	//a.Assert(list.Len() == 0)
	//a.Equal(list.Find(0), nil)
	...
	// Add some values. Value can be anything.
	list.Set([]byte("name"), "hello world")
	list.Set([]byte("name1"), 56)
	list.Set([]byte("name2"), 90.12)

	// Get element by index.
	elem := list.Get([]byte("name1")) // Value is stored in elem.Value.
	a.Equal(56, elem.Value)
	fmt.Println(elem.Value) // Output: 56
	next := elem.Next()     // Get next element.
	prev := next.Prev()     // Get previous element.
	//a.Equal(prev, "hello world")
	//a.Equal(next.Value, 90.12)
	fmt.Println(next.Value, prev.Value) // Output: 90.12    56

	// Or, directly get value just like a map
	val, ok := list.GetValue([]byte("name1"))
	fmt.Println(val, ok) // Output: 56  true

	// Find first elements with score greater or equal to key
	foundElem := list.Find([]byte("name2"))
	fmt.Println(string(foundElem.Key().([]byte)), foundElem.Value) // Output: 34 56

	// Remove an element for key.
	list.Remove([]byte("name"))
	unfoundElem := list.Find([]byte("name"))
	fmt.Println(unfoundElem)
	//fmt.Println(foundElem.Key(), foundElem.Value) // Output: 34 56
}
