...
func (r Rcoder) Less(t Rcoder) bool {
	return bytes.Compare(r.Key, t.Key) == -1
}

type Manager struct {
	BPTreeIdx map[string]*BTree[Rcoder]
}
...
//		m.BPTreeIdx[bucket] = New[Rcoder](8)
//	}
//
//	_, ok := m.BPTreeIdx[bucket].ReplaceOrInsert(Rcoder{Key: key, Value: value})
//	if !ok {
...
//	//TODO implement me
//	if tree, ok := m.BPTreeIdx[bucket]; ok {
//		item, ok := tree.Get(Rcoder{Key: key})
//		if !ok {
